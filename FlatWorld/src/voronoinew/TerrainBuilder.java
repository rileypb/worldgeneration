...
public class TerrainBuilder {
	...
	public void normalizeElevations(Graphs buildResult) {
		OptionalDouble average = buildResult.dualVertices.stream().mapToDouble((v) -> {
			return v.elevation;
		}).average();
	...
	public void fillDepressions(Graphs graphs) {
		...
		while (isChanged) {
			isChanged = graphs.dualVertices.stream().reduce(Boolean.FALSE, (c, v) -> {
				...
				if (min >= v.pdElevation && v.pdElevation < Double.POSITIVE_INFINITY) {
					//					System.out.println("change " + v.elevation + ", min = " + min);
					v.pdElevation = min + 0.001;
					return true;
				} else {
					v.pdElevation = v.elevation;
				}
	...
	public void runRivers(Graphs graphs) {
		...
		graphs.voronoiVertices.forEach((loc) -> {
			// find min height neighbor and add directed edge to it.
		});
	}
